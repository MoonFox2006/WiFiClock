...
void parseEpochTime(uint32_t epoch, uint8_t *hour, uint8_t *minute, uint8_t *second) {
  if (second)
    *second = epoch % 60;
  epoch /= 60;
  if (minute)
...
void parseEpochDate(uint32_t epoch, uint8_t *weekday, uint8_t *day, uint8_t *month, uint16_t *year) {
  uint16_t days = epoch / (60 * 60 * 24);
  uint16_t y;
  bool leap;
